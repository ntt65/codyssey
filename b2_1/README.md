...
### 🏛️ 가계부 애플리케이션의 4대 역할 분담
  ...
  ----------------|----------------------|-----------------------------------------------------|-----------------------------------------------------
   cli.py | 프레젠테이션 (UI)    | 사용자 인터랙션- 터미널 프롬프트 대기- 키보드       | 가게의 점원 (얼굴)(손님과 대화하고, 주문을 받아
                  |                      | 자동완성 (Tab) 및 한글 오타 방지- 입력받은 값       | 정돈하고, 완성된 음식을 예쁘게 플레이팅해서
                  |                      | 정돈하여 서비스에 전달- 표(Table) 예쁘게 줄 맞춰    | 서빙하는 역할)
                  |                      | 렌더링하기                                          |
   service.py | 도메인 서비스        | 검증 및 계산 (핵심 기능)- 날짜, 금액, 카테고리      | 가게의 요리사 (두뇌)(UI가 대화형 셸인지 단발
                  | (비즈니스)           | 유효성 정밀 검증 ( validate_fields )- 예산 초과     | CLI인지 상관없이, 전달된 재료가 상하지
                  |                      | 경고율 계산- 정기 반복 거래 중복 체크 차단- CSV     | 않았는지(검증) 검사하고 실제로 요리(비즈니스
                  |                      | 파싱 조율                                           | 계산)하는 역할)
   repository.py | 인프라스트럭처       | 물리적 파일 입출력- JSONL 파일에 읽고 쓰기- 임시    | 가게의 창고 관리자 (손)(냉장고나 창고에 식자재를
                  | (영속성)             | 파일을 이용한 원자적 수정/삭제- 디렉터리 폴더 자동  | 안전하게 보관하고 필요할 때 하나씩 꺼내주는 역할)
                  |                      | 생성                                                |
   models.py | 도메인 모델 (DTO)    | 데이터 규격 (DNA)-  Transaction  및                 | 요리 레시피 규격서 (DNA)(손님 주문서나 식자재 보관
                  |                      | RecurringTemplate  형태 정의- 객체를 딕셔너리로     | 명세서의 고정된 서식 규격)
                  |                      | 변환/복원                                           |
  ──────
  ### 💡 이렇게 분리해서 얻는 이점
