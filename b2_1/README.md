...
### 🏛️ 가계부 애플리케이션의 4대 역할 분담

| 파일명 | 계층 (Layer) | 주된 책임 (역할) | 현실 비유 |
| :--- | :--- | :--- | :--- |
| **[__main__.py](file:///Users/mpeg46551/codyssey/b2_1/budget_app/__main__.py)** | 부트스트랩 / 진입점 (Entry) | 프로그램 구동 및 의존성 주입 조립<br>- 명령줄 인자(argparse) 파싱<br>- FileRepository, BudgetService, InteractiveShell 조립<br>- 실행 모드(CLI vs 대화형) 분기 라우팅 | **가게의 지배인 / 매장 개업 (지배인)**<br>(가게 문을 열고, 점원/요리사/창고 관리자를 조립 배치하며, 고객의 주문 형태(CLI/대화형)에 맞춰 안내하는 역할) |
| **[cli.py](file:///Users/mpeg46551/codyssey/b2_1/budget_app/cli.py)** | 프레젠테이션 (UI) | 사용자 인터랙션<br>- 터미널 프롬프트 대기<br>- 키보드 자동완성 (Tab) 및 한글 오타 방지<br>- 입력받은 값 정돈하여 서비스에 전달<br>- 표(Table) 예쁘게 줄 맞춰 렌더링하기 | **가게의 점원 (얼굴)**<br>(손님과 대화하고, 주문을 받아 정돈하고, 완성된 음식을 예쁘게 플레이팅해서 서빙하는 역할) |
| **[service.py](file:///Users/mpeg46551/codyssey/b2_1/budget_app/service.py)** | 도메인 서비스 (비즈니스) | 검증 및 계산 (핵심 기능)<br>- 날짜, 금액, 카테고리 유효성 정밀 검증 (`validate_fields`)<br>- 예산 초과 경고율 계산<br>- 정기 반복 거래 중복 체크 차단<br>- CSV 파싱 조율 | **가게의 요리사 (두뇌)**<br>(UI가 대화형 셸인지 단발 CLI인지 상관없이, 전달된 재료가 상하지 않았는지(검증) 검사하고 실제로 요리(비즈니스 계산)하는 역할) |
| **[repository.py](file:///Users/mpeg46551/codyssey/b2_1/budget_app/repository.py)** | 인프라스트럭처 (영속성) | 물리적 파일 입출력<br>- JSONL 파일에 읽고 쓰기<br>- 임시 파일을 이용한 원자적 수정/삭제<br>- 디렉터리 폴더 자동 생성 | **가게의 창고 관리자 (손)**<br>(냉장고나 창고에 식자재를 안전하게 보관하고 필요할 때 하나씩 꺼내주는 역할) |
| **[models.py](file:///Users/mpeg46551/codyssey/b2_1/budget_app/models.py)** | 도메인 모델 (DTO) | 데이터 규격 (DNA)<br>- `Transaction` 및 `RecurringTemplate` 형태 정의<br>- 객체를 딕셔너리로 변환/복원 | **요리 레시피 규격서 (DNA)**<br>(손님 주문서나 식자재 보관 명세서의 고정된 서식 규격) |
  ### 💡 이렇게 분리해서 얻는 이점

  이렇게 나누어 두면, 나중에 **"터미널 UI( cli.py ) 대신 예쁜 웹 브라우저 UI(HTML/JS)로 가계부를 바꾸고 싶다"**고 할 때, 비즈니스 계산 로직인
  service.py 나 파일 저장소인  repository.py 는 단 한 줄도 고치지 않고  cli.py 만 다른 UI 모듈로 쏙 교체하면 되기 때문에 프로그램이 매우 유연하고
  튼튼해집니다.
