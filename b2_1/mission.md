# 💰 AI/SW 기초: Python과 Git 심화 미션 명세서

## [과제] 파일 기반 가계부 콘솔 프로그램 만들기
...
* `update`는 “옵션 방식 또는 대화형” 중 하나를 선택해도 되나, 문서에 명확히 고정해야 합니다.
...
* `summary` 실행 시 예산이 설정되어 있으면 아래 항목을 출력합니다.
  * 예산 대비 사용률 (%)
  * 초과 여부 (초과 시 경고 문구)
...
* **라이브러리 제약**:
  * 표준 라이브러리만 사용 가능
  * 별도 `pip install`이 필요한 외부 라이브러리 사용 절대 금지
* **저장 방식**:
